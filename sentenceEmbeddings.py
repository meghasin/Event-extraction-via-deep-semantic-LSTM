...
"""
Generates document embeddings for sentences in Stanford CoreNLP parsed 
documents
"""
...
def main(inputDir, modelFileName, outFileName):
	"""
	Generates a vector per sentence per document based on the 
	given doc2vec model
	"""
	#load the model
	...
	for name in listdir(inputDir):
		...
		#make an embedding per sentence
		for sentence in doc.sentences:
			
			#get the sentence text and generate the vector
			vec = model.infer_vector(sentence.words(), alpha=startAlpha, steps=inferEpoch)

			#save the results
			out.write("{},{}: {}\n".format(doc.id, sentence.id, ", ".join(map(str,vec))))
